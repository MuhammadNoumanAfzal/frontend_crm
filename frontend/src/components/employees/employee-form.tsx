...
import type { EmployeeRow, User } from "@/lib/api/types";
...
export function EmployeeForm({ mode, initialData, managers, onSaved }: EmployeeFormProps) {
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      employee_code: initialData.employee_data?.employee_code ?? "",
      department: initialData.employee_data?.department ?? "",
      designation: initialData.employee_data?.designation ?? "",
      employment_type: initialData.employee_data?.employment_type ?? "FULL_TIME",
      joining_date: asDateInput(initialData.employee_data?.joining_date),
      employment_status: initialData.employment_status ?? "ACTIVE",
      phone: initialData.employee_data?.phone ?? "",
      salary_flat: String(initialData.employee_data?.salary_flat ?? ""),
      currency: initialData.employee_data?.currency ?? "AED",
      pay_frequency: initialData.employee_data?.pay_frequency ?? "MONTHLY",
      personal_email: initialData.employee_data?.personal_email ?? "",
      probation_end_date: asDateInput(initialData.employee_data?.probation_end_date),
      contract_end_date: asDateInput(initialData.employee_data?.contract_end_date),
      date_of_birth: asDateInput(initialData.employee_data?.date_of_birth),
      gender: initialData.employee_data?.gender ?? "",
      nationality: "",
      emergency_contact_name: initialData.employee_data?.emergency_contact_name ?? "",
      emergency_phone: initialData.employee_data?.emergency_phone ?? "",
      bank_name: initialData.employee_data?.bank_name ?? "",
      bank_account_no: initialData.employee_data?.bank_account_no ?? "",
      bank_iban: initialData.employee_data?.bank_iban ?? "",
      passport_number: initialData.employee_data?.passport_number ?? "",
      passport_expiry: asDateInput(initialData.employee_data?.passport_expiry),
      visa_type: initialData.employee_data?.visa_type ?? "",
      visa_number: initialData.employee_data?.visa_number ?? "",
      visa_expiry: asDateInput(initialData.employee_data?.visa_expiry),
      work_permit_number: initialData.employee_data?.work_permit_number ?? "",
      reporting_manager_id: initialData.employee_data?.reporting_manager_id ?? "",
      bitrix_id: initialData.bitrix_id ?? "",
      housing_allowance: String(initialData.employee_data?.housing_allowance ?? "0"),
      transport_allowance: String(initialData.employee_data?.transport_allowance ?? "0"),
      other_benefits: String(initialData.employee_data?.other_benefits ?? "0"),
    });
  ...
  const submit = form.handleSubmit(async (values) => {
    ...
    const payload = {
      ...
      reporting_manager_id: clean.reporting_manager_id || undefined,
    };
  ...
  return (
    <Card className="shadow-sm">
      <CardHeader>
        <CardTitle className="text-xl font-bold tracking-tight">{isEdit ? "Edit Employee" : "Add New Employee"}</CardTitle>
      </CardHeader>
      <CardContent>
